...
All year-specific ingestion rules are read from ``data/metadata/df_metadata.xlsx``,
which is generated by ``build_metadata.py``. Raw fixed-width records are parsed
directly as bytes, only the required fields are extracted, missing-income
sentinels are removed, and household income is converted to per-capita income
when household-size metadata are available.
"""
...
def load_metadata(metadata_path=DEFAULT_METADATA_PATH):
    """Load and validate the metadata required for raw PNAD ingestion."""
    metadata_path = Path(metadata_path)

    if not metadata_path.is_file():
        raise FileNotFoundError(
            f"Metadata não encontrado: {metadata_path}. "
            "Execute src/build_metadata.py primeiro."
        )
    ...
    if not df_metadata["ano"].is_unique:
        raise ValueError("Metadata contém anos duplicados.")

    return df_metadata


def get_available_specs(df_metadata):
    """Return only years with a complete raw-file ingestion specification."""
...
def get_source_files(spec, raw_path=DEFAULT_RAW_PATH):
    """Resolve and validate the raw files declared by one metadata row."""
...
def _parse_int_field(line, start, end):
    """Parse one integer field from a raw byte line."""
...
def read_year_fast(spec, raw_path=DEFAULT_RAW_PATH, show_file_progress=True):
    """Read, filter, and transform one survey year from fixed-width raw files."""
    year = int(spec.ano)
...
def generate_datasets(
    ...
    """Generate all refined annual Parquet files and return a summary table."""
    raw_path = Path(raw_path)
    ...
    if len(df_summary) != len(available):
        raise RuntimeError("Resumo inconsistente com o número de anos processados.")
    if not df_summary["ano"].is_unique:
        raise RuntimeError("Resumo contém anos duplicados.")
