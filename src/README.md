...
<p align="justify">Two explicit analytical tables are generated by this stage: <code>trusted_trim_audit.csv</code> contains the cutoff, removal counts, distribution statistics and execution diagnostics, while <code>trusted_distribution_tests.csv</code> records the Boolean invariants and comparative changes for every annual distribution. Both files are written to <code>assets/tables_analysis</code>.</p>
