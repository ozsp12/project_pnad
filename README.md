# PNAD and PNAD Contínua Household Income Research

## Overview

This repository contains the reproducible data-construction and analysis workflow for a record-level household-income dataset derived from the Brazilian *Pesquisa Nacional por Amostra de Domicílios* (PNAD) and *PNAD Contínua*. The project covers all 45 available survey years between 1976 and 2025 and preserves the explicit survey gaps in 1980, 1991, 1994, 2000, and 2010.

The released records are repeated annual cross-sections rather than a longitudinal panel. The workflow standardizes year-specific income fields into a common analytical structure, preserves a minimally transformed `refined` baseline, constructs a quality-controlled `trusted` benchmark, and produces documented cross-year data products suitable for scientific reuse and archival deposition.
...
| 01 | raw fixed-width records → standardized annual baseline | `data/refined/` |
...
The dataset title is **PNAD and PNAD Contínua Household Income Microdata for Brazil, 1976–2025**. The archived package contains the two consolidated Parquets, their variable-level schemas, annual metadata, dataset-level metadata, a package README, and SHA-256 checksums. The files included in the deposited package are treated as frozen archival objects; documentation corrections made after deposition do not silently modify those checksummed files.

Citation metadata for the repository software are provided in [`CITATION.cff`](CITATION.cff). The repository software is released under the MIT License, while the Zenodo dataset has its own dataset-level licensing metadata. Original PNAD and PNAD Contínua source microdata remain attributable to and governed by the Instituto Brasileiro de Geografia e Estatística (IBGE).
