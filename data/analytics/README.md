...
The `data/analytics` directory contains the canonical cross-year record-level products prepared by Stage 04 for scientific reuse and archival deposition. Stage 04 validates and concatenates the annual `refined` and `trusted` layers without applying additional statistical filtering or re-estimating scientific models.
...
The 2015–2016 PNAD/PNAD Contínua transition remains a methodological comparability boundary. Users requiring population-representative inference or survey-design reconstruction should return to the original IBGE microdata and corresponding annual documentation.
