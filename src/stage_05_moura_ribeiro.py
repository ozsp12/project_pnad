...
def _save_figure(fig, stem: str):
    """Persist the repository's canonical paper-figure format (SVG only)."""
...
def _load_tables():
    ...
    required_metadata = {"ano", "Currency", "Exchange"}
...
def build_table_01(stats, metadata):
    t = metadata[["year", "currency", "exchange"]].merge(
        stats[["year", "mean_nominal"]], on="year", how="inner"
    )
    t["mean_income_current_usd"] = t["mean_nominal"] / t["exchange"]
    out = t.rename(
        columns={
            "currency": "currency_name_symbol",
            "exchange": "local_currency_units_per_usd",
        }
    )[[
        "year",
        "currency_name_symbol",
        "local_currency_units_per_usd",
        "mean_income_current_usd",
    ]]
    out.to_csv(
        TABLES_PAPER / "moura_ribeiro_2009_table_01_currency_mean_income_1978_2025.csv",
        index=False,
    )
    return out
...
def build_table_02(annual):
    t = annual.copy()
    t["correlation_coefficient"] = np.sqrt(
    ...
    out.to_csv(
        TABLES_PAPER / "moura_ribeiro_2009_table_02_gompertz_parameters_1978_2025.csv",
        index=False,
    )
    return out
...
def build_table_03(annual):
    t = annual.copy()
    t["ls_correlation_coefficient"] = np.sqrt(
    ...
    out = t[[
        ...
        "ls_correlation_coefficient",
        "pareto_population_pct",
    ]].copy()
    out.to_csv(
        TABLES_PAPER / "moura_ribeiro_2009_table_03_pareto_parameters_1978_2025.csv",
        index=False,
    )
    return out
...
def build_table_04(stats, annual):
    ...
    out.to_csv(
        TABLES_PAPER / "moura_ribeiro_2009_table_04_income_shares_gini_1978_2025.csv",
        index=False,
    )
    return out
...
def plot_ccdf(curves, years, stem, ncols):
    ...
    _finish_grid(
        ...
        "Complementary cumulative distribution, $F(x)$ (%)",
    )
...
def plot_lorenz(lorenz, years, stem, ncols):
    ...
    for ax, year in zip(axes.ravel(), years):
        ...
        ax.plot(
            100.0 * d["population_share"],
            100.0 * d["income_share"],
            linewidth=1.1,
        )
...
def build_exponential_fits(curves, annual):
    ...
    for year in _filtered_years(annual["year"].dropna().astype(int)):
        ...
        intercept, slope, r2 = _linear_fit(
            d["income_normalized"], np.log(d["empirical_ccdf_percent"])
        )
        rows.append(
            {
                "year": year,
                "exp_intercept": intercept,
                "exp_lambda": -slope,
                "exp_r2": r2,
                "x_max": xg,
            }
    ...
    out.to_csv(
        TABLES_PAPER / "moura_ribeiro_2009_exponential_fit_diagnostics_1978_2025.csv",
        index=False,
    )
    return out
...
def plot_pareto(curves, annual, years, stem, ncols, method):
    ...
    for ax, year in zip(axes.ravel(), years):
        fit = annual_i.loc[year]
        if method == "ls":
            xmin = float(fit["pareto_x_pmin"])
            fitted_col = "pareto_fitted_ccdf_percent_ls"
        else:
            xmin = float(fit["transition_x_t"])
...
def load_world_bank_gdp_growth():
    """Load the pinned World Bank/WDI GDP-growth snapshot without network access."""
    gdp = pd.read_csv(GDP_GROWTH_PATH)
    required = {"year", "gdp_growth_pct", "indicator", "source"}
    missing = required.difference(gdp.columns)
    if missing:
        raise ValueError(
            "GDP auxiliary series is missing required fields: "
            + ", ".join(sorted(missing))
        )
    ...
    if not np.isfinite(gdp["gdp_growth_pct"]).all():
        raise ValueError("GDP auxiliary series contains non-finite growth values.")

    out = gdp[(gdp["year"] >= START_YEAR) & (gdp["year"] <= END_YEAR)].copy()
    out = out.sort_values("year").reset_index(drop=True)
    out[["year", "gdp_growth_pct"]].to_csv(
        TABLES_PAPER / "moura_ribeiro_2009_figure_15_gdp_growth_1978_2025.csv",
        index=False,
    )
    return out
...
def validate(table02, table03, table04):
    merged = table02[["year", "gompertz_x_gmax"]].merge(
        table03[["year", "pareto_x_pmin", "transition_x_t"]],
        on="year",
        how="inner",
    )
    bad = merged[
        (merged["transition_x_t"] < merged["gompertz_x_gmax"] - 1e-12)
        | (merged["transition_x_t"] > merged["pareto_x_pmin"] + 1e-12)
    ]
    if not bad.empty:
        raise AssertionError(
            "Threshold ordering failed for years: "
            + ", ".join(map(str, bad["year"].tolist()))
        )
    if not np.allclose(
        table04["gompertz_income_share_pct"] + table04["pareto_income_share_pct"],
        100.0,
        atol=1e-8,
    ):
...
def main():
    ...
    plot_exponential(
        ...
        expfits,
        early,
        "moura_ribeiro_2009_figure_06_exponential_1978_1990",
        ncols=3,
    )
    plot_exponential(
        curves,
        expfits,
        late,
        "moura_ribeiro_2009_figure_07_exponential_1992_2025",
        ncols=5,
    )
    ...
    plot_gompertz(
        ...
        annual,
        early,
        "moura_ribeiro_2009_figure_08_gompertz_1978_1990",
        ncols=3,
    )
    plot_gompertz(
        curves,
        annual,
        late,
        "moura_ribeiro_2009_figure_09_gompertz_1992_2025",
        ncols=5,
    )
    ...
    plot_pareto(
        ...
        annual,
        early,
        "moura_ribeiro_2009_figure_10_pareto_ls_1978_1990",
        ncols=3,
        method="ls",
    )
    plot_pareto(
        ...
        annual,
        late,
        "moura_ribeiro_2009_figure_11_pareto_ls_1992_2025",
        ncols=5,
        method="ls",
    )
    plot_pareto(
        ...
        annual,
        early,
        "moura_ribeiro_2009_figure_12_pareto_mle_1978_1990",
        ncols=3,
        method="mle",
    )
    plot_pareto(
        ...
        annual,
        late,
        "moura_ribeiro_2009_figure_13_pareto_mle_1992_2025",
        ncols=5,
        method="mle",
    )

    table04 = build_table_04(stats, annual)
    plot_pareto_share(table04)
    gdp = load_world_bank_gdp_growth()
    plot_gdp_growth(gdp)
    ...
    manifest.to_csv(
        TABLES_PAPER / "moura_ribeiro_2009_replication_manifest.csv",
        index=False,
    )
    print(
        "Generated Moura-Ribeiro replication assets for "
        f"{len(years)} survey years ({years[0]}-{years[-1]})."
    )
